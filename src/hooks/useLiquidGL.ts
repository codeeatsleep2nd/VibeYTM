        ...
        if (targets.length > 0) {
          try {
            ...
            const promoteToWebGL = (): void => {
              // Add a global flag class to <html> so a CSS rule can
              // disable the `liquid-glass-*` fallback backgrounds in one
              // shot. Without this, the CSS class's bg + backdrop-filter
              // sits on TOP of the WebGL canvas (which paints at
              // z = lensZ - 1) and occludes the refraction. The CSS
              // rules that consume this flag live in `global.css` —
              // grep for `:root.liquidGL-active`.
              document.documentElement.classList.add('liquidGL-active');
            };
