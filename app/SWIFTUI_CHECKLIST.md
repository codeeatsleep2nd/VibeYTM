...
## Launch — NSWindow timing rules

Hours-long debug sessions (2026-05) burned through three classes of
launch regression. The rules below codify each fix. Do not violate them.

- [ ] **NEVER create NSWindow / NSPanel instances in `@State` property initializers or `App.init()` on macOS 26.** SwiftUI's WindowGroup primary window is set up during the same launch phase; any sibling AppKit window created here prevents the WindowGroup from ever reaching `onScreen=true`. Symptom: process launches, all windows materialize in CGWindowList, but every user-facing window stays `onScreen=false`, no dock icon, no menu bar — app effectively invisible. **Defer all such window creation to the WindowGroup's `.task { }` modifier.** Concretely: `BridgeHost.start()` (which creates a hidden NSWindow for its WKWebView) is called from `bootstrap.startBridgeIfNeeded()` inside `.task { }`, NOT from `AppBootstrap.init()`. Same for any future window-creating subsystem (tray, About panel, etc.). Captured by `app/build.sh`'s visible-window smoke test (`bash app/build.sh --install` fails if no visible main window appears within 5 s — set `VERIFY_LAUNCH=0` to opt out in CI).
- [ ] **NEVER declare an entitlement requiring Developer ID provisioning in the ad-hoc-signed bundle** (App Group `com.apple.security.application-groups`, iCloud `com.apple.developer.icloud-services`, Push `aps-environment`, etc.). macOS Tahoe treats a declared-but-unprovisioned entitlement as a per-launch user-trust prompt ("VibeYTM would like to access data beyond its own folder") that ALSO intercepts the launch flow in ways that can hide the main window. Re-add the entitlement AT THE SAME TIME you switch to Developer ID signing (paid Apple Developer Program). The block to add lives in `app/build.sh`'s inline entitlements + `app/VibeYTM.entitlements` — both have a commented stub showing the exact lines.
- [ ] **AuthWebView's nav delegate MUST fire `onAuthenticated` when it lands on music.youtube.com.** Two separate WKWebView instances (visible AuthWebView + hidden BridgeHost) share cookies via `WKWebsiteDataStore.default()` — but cookies alone don't reload the bridge's cached signed-out page. The Coordinator at `AuthWebView.swift` calls back via `bootstrap.reloadBridgeForAuth()`, which calls `BridgeHost.reload()`. Without this wiring the app stays stuck on AuthScaffold forever even after the user signs in. Test: sign out → relaunch → sign in via visible WebView → within 5 s, the sidebar bottom must switch from "Not signed in" to the user's name AND the detail pane must switch from the YTM webview to the native SwiftUI HomeView. If either step fails the auth-sync wiring is broken.
...
- [ ] **Build clean + smoke test passes**: `bash app/build.sh --install` — ad-hoc signed `.app` bundle assembled, signature verified, copied to `~/Applications`, AND the built-in launch smoke test prints `✓ smoke test passed — visible main window detected`. The smoke test launches the app and asserts via `CGWindowListCopyWindowInfo` that a non-audio-engine VibeYTM window has `onScreen=true` within 5 s. If it FAILS, fix the launch regression before doing anything else — the diagnostic output names the three likely causes (NSWindow-in-init, entitlement-vs-signing prompt, scene body hang). Opt out with `VERIFY_LAUNCH=0` ONLY in CI without a windowserver session.
...
- [ ] **Auth-sync test (cold sign-in)**: sign out (clear `~/Library/WebKit/com.vibeytm.dev/` if needed for a true cold start), launch the app, sign in via the visible AuthWebView. Within 5 s of completing the Google sign-in: (a) the sidebar bottom must change from "Not signed in" to the user's name + avatar, and (b) the detail pane must switch from the YTM web UI to the native SwiftUI HomeView. If either fails, `AuthWebView` → `onAuthenticated` → `bootstrap.reloadBridgeForAuth()` → `BridgeHost.reload()` wiring is broken. Regression captured in `/tmp/vibeytm-current-state.png` (Sprint 0-5 era) — never let this ship again.
