...
(function () {
  ...
  window.__VIBEYTM_COMMAND__ = function (cmd, args) {
    ...
    switch (cmd) {
      ...
      case 'load_video_id':
        // Load + auto-play a videoId via YouTube's IFrame Player API.
        // Bypasses YTM's polymer-router click handling, which gates
        // auto-play on user-gesture (`isTrusted`). Programmatic anchor
        // clicks have isTrusted=false, so polymer routes to the track
        // but never starts playback. loadVideoById respects the
        // mediaTypesRequiringUserActionForPlayback=[] override and
        // begins playback unconditionally.
        if (args && typeof args.videoId === 'string' && args.videoId) {
          try {
            player.loadVideoById({
              videoId: args.videoId,
              startSeconds: typeof args.startSeconds === 'number' ? args.startSeconds : 0
            });
            setStatusOptimistic('buffering');
          } catch (e) {
            log('load_video_id failed: ' + (e && e.message ? e.message : e));
          }
        }
        break;
      case 'seek':
