...
Always perform a `gh search repos` / `gh search code` query, or a targeted web search, BEFORE writing the plan — not after the user pushes back. Cite the references you used in the plan so future-you can re-verify. If no existing implementation matches, document what you searched for so the absence is recorded, not assumed.
